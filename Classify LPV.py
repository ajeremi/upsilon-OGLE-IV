...
loadcat = sorted(glob.glob('LPV*'))
for name in loadcat:
	catmag, catperiod = np.loadtxt(fname=name, delimiter='\t', unpack=True, usecols=(6,7))
	IDs, modes, evols, specs = np.genfromtxt(fname=name, dtype=np.str, unpack=True, usecols=(0,3,4,5))
	ID = [x for x in IDs]
	varmode = [y for y in modes]
	evo = [r for r in evols]
...
for f in filenames:
	...
	thetrue = catstring + varmode[wherestar] + '_' + evo[wherestar] + '_' + spec[wherestar]
	if varmode[wherestar]=='OSARG':
		thetrue = thetrue[:-7]
	else:
		thetrue = thetrue[:-5]
