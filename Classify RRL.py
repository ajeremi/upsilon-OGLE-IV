...
for name in ab:
	...
	vars = ['RRL_ab' for x in pf]
	varmode.extend(vars)

ceh = sorted(glob.glob('RRc.*'))
for name in ceh:
	pf = np.genfromtxt(fname=name, delimiter=',', usecols=(3))
	IDfs = np.genfromtxt(fname=name, delimiter=',', dtype=np.str, usecols=(0))
	pf = np.nan_to_num(pf)
	...
	vars = ['RRL_c' for x in pf]
	varmode.extend(vars)

deh = sorted(glob.glob('RRd.*'))
for name in deh:
	pf = np.genfromtxt(fname=name, delimiter=',', usecols=(3))
	IDfs = np.genfromtxt(fname=name, delimiter=',', dtype=np.str, usecols=(0))
	pf = np.nan_to_num(pf)
	...
	vars = ['RRL_d' for x in pf]
	varmode.extend(vars)


adeh = sorted(glob.glob('aRRd.*'))
for name in adeh:
	pf = np.genfromtxt(fname=name, delimiter=',', usecols=(3))
	IDfs = np.genfromtxt(fname=name, delimiter=',', dtype=np.str, usecols=(0))
	pf = np.nan_to_num(pf)
	...
	vars = ['aRRL_d' for x in pf]
	varmode.extend(vars)


print "All catalogues are loaded. Let's go."

# - - - L O A D - F I L E - - -
filenames = sorted(glob.glob('OGLE-LMC-RRLYR-*.dat'))
...
for f in filenames:
	...
	thetrue = varmode[wherestar]	
	#thepred = label.translate(None, 'L_')
