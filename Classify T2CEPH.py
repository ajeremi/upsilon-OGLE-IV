...
catperiod = np.genfromtxt(fname='t2cep.dat', delimiter=',', usecols=(3))
IDs = np.genfromtxt(fname='t2cep.dat', delimiter=',', dtype=np.str, usecols=(0))
ID = [x for x in IDs]

# - - - L O A D - F I L E - - -	
filenames = sorted(glob.glob('OGLE-BLG-T2CEP*.dat'))
...
for f in filenames:
	...
	if len(m)<101:
		...
		plt.errorbar(fase,m,yerr=e,linestyle='none',marker='o',ms=6,
		 elinewidth=1, mfc='#009874', ecolor='#95a5a6', capsize=30) 
